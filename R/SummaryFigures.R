...
forest(jonsinvyearRMA,efac=c(2,2,2),slab=NA,digits=0,main="",xlab=expression(paste("Change in abundance (inds. m"^-3," year"^-1,")")),mlab="Overall",top=1,col=cols[1],psize=0.1)
text(par()$usr[1],16:1,jonsinvMaxAbRegdf$Species,font=3,pos=4)
...
forest(birdyearRMA,xlim=c(-3,1.8),efac=c(3,3,4),slab=NA,xlab=expression(paste("Change in terrortories (km"^-2," year"^-1,")")),main="",mlab="Overall",top=1,col=cols[1],psize=0.1)
text(par()$usr[1],5:1,c("Phylloscopus trochilus","Anthus trivialis","Luscinia svecica","Emberiza schoeniclus","Turdus iliacus"),font=3,pos=4)
...
forest(plantyearRMA,efac=c(2,2,2),slab=NA,main="",xlab=expression(paste("Change in peak flowering (days year"^-1,")")),mlab="Overall",top=1,psize=0.1,col=cols[4])
text(par()$usr[1],22:1,plantefregyear$Species,font=3,pos=4)
